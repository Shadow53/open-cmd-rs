    ...
    pub fn uri(&self) -> Result<Url> {
        match self {
            ...
            Self::Path(path) => {
                ...
                .map_err(|_| Error::FileToURI(path.clone()))
            }
        }
    }
}
...
mod tests {
    ...
    fn test_to_uri() {
        ...
        let path_uri = Url::from_file_path(std::env::current_dir().unwrap().join(&path).clean()).unwrap();
